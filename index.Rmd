...
ezGenomeTracks makes it easy to create publication-quality genomic visualizations with minimal code. Built entirely on ggplot2, it provides both high-level `ez_*()` wrappers for quick plotting and low-level `geom_*()` layers for full customization.
...
```{r example-quick, fig.height=3, fig.width=10}
...
ezGenomeTracks supports **7 track types** covering the most common genomic visualization needs:
...
**For quick plots**, use `ez_*()` functions that handle data import, theming, and scaling automatically:
...
- **Publication themes**: Clean, minimal defaults with `ez_*_theme()` functions
...
```{r example-multi, fig.height=4, fig.width=10}
...
- [Get Started](articles/get_started.html) - Core concepts and design principles
...
If ezGenomeTracks aids your research, please cite the GitHub repository in your methods section.
