...
GeomGene <- ggplot2::ggproto(
  ...
  draw_panel = function(
    ...
    if (nrow(gene_data) > 0) {
      ...
      if (!"linewidth" %in% names(body_data)) {
        body_data$linewidth <- intron_width
      }
...
process_gene_data <- function(
  ...
  if (nrow(genes) == 0) {
    stop("No gene features could be constructed from the input")
  }
  ...
  has_exon <- any(gr_df[[type]] == "exon") &&
    all(c("start", "end") %in% names(gr_df))
  if (has_exon) {
    exons <- gr_df[
      gr_df[[type]] == "exon",
      c(gene_id, gene_name, "strand", "start", "end")
