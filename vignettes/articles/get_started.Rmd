...
There have been several R/python packages that plot genome browser tracks, but most of them require a lot of coding to set up. PygenomeTracks is one of my favorite, but it requires writing `.ini` configuration files and running command-line tools, which can be cumbersome for R users, and is not flexible for generating a large number of plots in exploratory data analysis.

Using `ggplot2`, it is possible to generate beautiful genome browser tracks, although it typically requires a lot of custom code and data wrangling. That motivated me to create an R package that is based on `ggplot2`, while providing simple high-level functions for common track types. The goal is to make it easy to create publication-quality genome browser plots with minimal code for most users, while still allowing for advanced customization for a smaller number of power users.
...
cov <- ez_coverage(
  "inst/extdata/example_signal.bedgraph",
  region = "chr1:1009000-1092000",
  colour = "steelblue"
)

# one-line gene model track
genes <- ez_gene(example_genes, region = "chr1:1009000-1092000")
